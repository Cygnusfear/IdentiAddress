...
class IdentiAddress {
    ...
    formatAddressBlocksEmoji(address) {
        ...
        let palette = this.palettes[
            (address.charCodeAt(2) +
                address.charCodeAt(8) +
                address.charCodeAt(15)) %
        ...
        for (let i = 2; i < address.length - 2; i += 2) {
            ...
            string += `<span style="color: ${rgb} !important;${underline}">${
                address[i + 1]
            }${address[i + 2]}${emoji[id].emoji}</span>`;
        }
        string += "</span>";
        return string;
    }
    ...
    formatAddressBlocksEmojiOnly(address) {
        ...
        for (let i = 2; i < address.length - 2; i += 2) {
            ...
            let id = Math.round((ca + cb + cc) % emoji.length);
            let rgb = palette[Math.round((ca + cb + cc) % palette.length)];
            string += `<span style="color: ${rgb} !important;${underline}; white-space:nowrap">${
                emoji[id].emoji
            }</span>`;
        }
        string += "</span>";
    ...
    formatAddressBlocksBorder(address) {
        ...
        let border = palette[Math.round(hashed.charCodeAt(3) % palette.length)];
        let string = `<span style="font-family: 'roboto mono'; font-weight: bolder !important; color: #000; margin: 0 2px; overflow: hidden;">0X</span><span style="border: 0px solid #aaa; border-radius: 5px;"><span style="font-family: 'roboto mono'; font-weight: lighter;">`;
        ...
        for (let i = 2; i < address.length - 1; i += 2) {
            ...
            let underline =
                ca % 3 > 0
                    ? `color: white !important; background: ${rgbastring}`
                    : `!important; color: white !important; background: #fefefe; color: #000 !important`;
    ...
    formatAddressBottomBorder(address) {
        //check if address is a properly formatted Ethereum address
        if (!address) return undefined;
        //each character is colored by its position plus ansii code
        let rgbs = [];
    ...
    formatAddressBottomBorderSmall(address) {
        //check if address is a properly formatted Ethereum address
        if (!address) return undefined;
        //each character is colored by its position plus ansii code
        let rgbs = [];
        ...
        let border = palette[Math.round(hashed.charCodeAt(3) % palette.length)];
        let string = `<span style="font-family: 'roboto mono'; font-weight: bolder !important; color: #000; margin: 0 2px;">0X</span><span style="border-bottom: 0px solid #aaa; border-radius: 5px;"><span style="font-family: 'roboto mono'; font-weight: lighter;">`;
        ...
        for (let i = 2; i < address.length - 1; i += 2) {
            ...
            let underline =
                ca % 3 > 0
                    ? `color: ${rgbastring}; border-bottom: 3px solid ${rgbastring}`
                    : `!important; border-bottom: 3px solid #fefefe;`;
            string += `<span style="position: relative; padding: 0 3px 0 3px; color: ${rgbastring};">${adresses}<span class="underline" style="width: 90%; position: absolute; left: 10%; bottom: -4px;${underline}"></span></span>`;
        }
        string += "</span></span>";
        return string;
    }
